#ifndef STDCHAIN_UTIL_ENCODE_SERIALIZE_H
#define STDCHAIN_UTIL_ENCODE_SERIALIZE_H
...
template<typename Stream>
inline void Serialize(Stream &s, char a) { ser_writedata8(s, a); }
...
template<typename Stream>
inline void Serialize(Stream &s, const std::span<const unsigned char> &span) {
    s.write(CharCast(span.data()), span.size());
}

template<typename Stream>
inline void Serialize(Stream &s, const std::span<unsigned char> &span) { s.write(CharCast(span.data()), span.size()); }

#ifndef CHAR_EQUALS_INT8

template<typename Stream>
inline void Unserialize(Stream &s, char &a) { a = ser_readdata8(s); }
...
template<typename Stream>
inline void Unserialize(Stream &s, std::span<unsigned char> &span) { s.read(CharCast(span.data()), span.size()); }
...
struct CustomUintFormatter {
    ...
    void Unser(Stream &s, I &v) {
        ...
        static_assert(std::numeric_limits<U>::max() >= MAX && std::numeric_limits<U>::min() <= 0,
...
#endif //STDCHAIN_UTIL_ENCODE_SERIALIZE_H
